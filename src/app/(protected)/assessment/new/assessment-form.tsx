...
import { lookupPatientByPhone, createOrUpdatePatient, getUniquePlaces } from "@/lib/actions/patients";
...
import { 
  ...
  AlertCircle, Lightbulb, X, Search, Plus, Trash2, MapPin
} from "lucide-react";
import { cn } from "@/lib/cn";
import type { Outlet, MasterTest, DietPlan } from "@prisma/client";
...
export default function AssessmentForm({ outlets, testsByCategory, dietPlans }: Props) {
  ...
  const canProceed = () => {
    ...
    if (step === 3) {
      if (needsDietPlan === "Yes" && !dietPlanId) return false;
      return !!outletId && !!resultReceivedAt && !!interactionAt;
    }
    return false;
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await createAssessment({
        ...
        outletId,
        dietPlanId: needsDietPlan === "Yes" ? dietPlanId : null,
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm ring-1 ring-slate-100 min-h-[400px]">
        ...
        {step === 3 && (
          <Step4Clinical
            ...
            dietPlans={dietPlans} dietPlanId={dietPlanId} setDietPlanId={setDietPlanId}
      ...
      <div>
        ...
        <div className="relative mt-1.5">
          ...
          <span className="absolute right-3 top-1/2 -translate-y-1/2">
            {lookingUp && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-500 uppercase">Searching…</span>}
            {existingPatient && !lookingUp && <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700 uppercase">Returning ✓</span>}
            {phone.length === 10 && !existingPatient && !lookingUp && <span className="rounded-full bg-green-50 px-2 py-0.5 text-[10px] font-bold text-green-700 uppercase">New Patient</span>}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="relative">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute z-50 mt-1 w-full rounded-xl border border-slate-200 bg-white shadow-xl overflow-hidden py-1">
              {suggestions.map((s) => (
                <button
                  key={s} type="button"
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className={cn(
          ...
          <div className="my-2 relative">
            <span className={cn("text-5xl font-black tabular-nums", !bmi ? "text-slate-200" : "text-slate-900")}>
              {bmi ? bmi.toFixed(1) : "—"}
            </span>
          </div>
          {bmi && (
...
function Step3Tests({ allMasterTests, selectedTests, toggleTest, addCustomTest, updateTestValue, removeTest, gender }: any) {
  const [search, setSearch] = useState("");
  const [isOpen, setIsOpen] = useState(false);
  const filteredMaster = useMemo(() => {
    if (!search) return [];
    return allMasterTests.filter((t: any) => t.name.toLowerCase().includes(search.toLowerCase())).slice(0, 5);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="relative group">
        ...
        <div className="relative">
          ...
          />
          {search && <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300"><X className="h-4 w-4" /></button>}
        </div>
          ...
          <div className="absolute z-50 mt-2 w-full rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden">
            <div className="p-2 space-y-1">
              {filteredMaster.map((t: any) => (
                <button
                  key={t.id} onClick={() => { toggleTest(t.name); setSearch(""); setIsOpen(false); }}
                  className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left hover:bg-slate-50"
                >
                  <div>
                    <p className="text-sm font-bold">{t.name}</p>
                    <p className="text-[10px] font-medium opacity-60 uppercase">{t.category}</p>
                  </div>
              ...
              {search && !filteredMaster.find((t: any) => t.name.toLowerCase() === search.toLowerCase()) && (
                ...
                >
                  ...
                  <div><p className="text-sm font-bold">Add Custom: "{search}"</p></div>
                </button>
              )}
            </div>
          </div>
      ...
      <div className="space-y-3 pt-2">
        {selectedTests.map((st: any) => {
          ...
          const min = master ? (gender === "FEMALE" ? master.femaleMin : master.maleMin) : null;
          const max = master ? (gender === "FEMALE" ? master.femaleMax : master.maleMax) : null;
          const val = parseFloat(st.value || "");
          const isLow = !isNaN(val) && min !== null && val < min;
          const isHigh = !isNaN(val) && max !== null && val > max;
          return (
            <div key={st.name} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between gap-4 mb-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-800">{st.name}</h3>
                  {master && <p className="text-[10px] font-bold text-slate-400 uppercase">{master.category}</p>}
                </div>
                <button onClick={() => removeTest(st.name)} className="text-slate-300 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
              </div>
              <div className="flex flex-wrap items-center gap-4">
                <input type="number" step="0.01" value={st.value || ""} onChange={(e) => updateTestValue(st.name, e.target.value)} placeholder="Value" className={`${INPUT} w-28`} />
                {min !== null && (
                  <div className="flex gap-2">
                    <span className="bg-slate-100 text-slate-500 px-2 py-1 rounded-lg text-[10px] font-bold">REF: {min}-{max}</span>
                    {isLow && <span className="bg-blue-600 text-white px-2 py-1 rounded-lg text-[10px] font-black">LOW</span>}
                    {isHigh && <span className="bg-red-600 text-white px-2 py-1 rounded-lg text-[10px] font-black">HIGH</span>}
                  </div>
                )}
              </div>
            </div>
...
function Step4Clinical({ outlets, outletId, setOutletId, needsDietPlan, setNeedsDietPlan, dietPlans, dietPlanId, setDietPlanId, variationResults, setVariationResults, dietPlanNotes, setDietPlanNotes, remarks, setRemarks, resultReceivedAt, setResultReceivedAt, interactionAt, setInteractionAt }: any) {
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div>
          ...
          <div className="mt-2 flex gap-2">
            {["Yes", "No", "Maybe"].map((v) => (
              <button
                key={v} type="button"
                onClick={() => { setNeedsDietPlan(v); if (v !== "Yes") setDietPlanId(""); }}
                className={cn(
                  "flex-1 rounded-xl border py-2.5 text-[10px] font-bold uppercase transition-all",
                  needsDietPlan === v ? "bg-teal-600 text-white border-teal-600" : "bg-white text-slate-400 border-slate-200"
                )}
              >
                {v}
              </button>
      ...
      {needsDietPlan === "Yes" && (
        <div className="animate-in slide-in-from-top-2 duration-300">
          <label className={LABEL}>Select Diet Plan *</label>
          <select value={dietPlanId} onChange={(e) => setDietPlanId(e.target.value)} className={`${INPUT} border-teal-200 bg-teal-50/30`}>
            <option value="">Select a plan...</option>
            {dietPlans.map((p: any) => (<option key={p.id} value={p.id}>{p.name} (₹{p.price})</option>))}
          </select>
        </div>
      )}
      <div className="grid grid-cols-2 gap-4">
